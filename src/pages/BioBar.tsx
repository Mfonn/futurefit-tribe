...
/* ── Curated Herbs — Coming Soon ── */
const curatedHerbs = [
  { name: "Ashwagandha", benefit: "Stress & cortisol balance", icon: "🌿" },
  { name: "Moringa", benefit: "Nutrient-dense energy", icon: "🍃" },
  { name: "Hibiscus", benefit: "Blood pressure & antioxidants", icon: "🌺" },
  { name: "Turmeric", benefit: "Anti-inflammatory powerhouse", icon: "✨" },
  { name: "Lemongrass", benefit: "Digestive calm & detox", icon: "🌾" },
  { name: "Ginger Root", benefit: "Circulation & immunity", icon: "🫚" },
];
    ...
    <div className="min-h-screen bg-background">
      ...
      <section ref={heroRef} className="relative min-h-[80vh] overflow-hidden">
        <div className="absolute inset-0">
          <img src={bioBarHeroImg} alt="The Bio Bar — Fresh Herbal Drinks" className="h-full w-full object-cover" />
        ...
        <div className="relative flex min-h-[80vh] items-end pb-24">
          <div className="section-padding w-full max-w-7xl mx-auto">
            <motion.div
              ...
              className="max-w-2xl"
            >
              <p className="mb-6 font-body text-sm tracking-[0.3em] text-primary">THE BIO BAR</p>
              <h1 className="mb-8 font-display text-5xl font-bold leading-[1.1] md:text-7xl">
                Simple Herbs,<br /><span className="glow-text">Real Results</span>
              </h1>
              <p className="max-w-lg font-body text-lg leading-relaxed text-muted-foreground">
                Fresh herbs. Familiar ingredients. Drinks made on-site that taste incredible and actually make you feel something. Come taste the difference — the recipes stay with us.
              </p>
            </motion.div>
          </div>
        </div>
      ...
      <section className="section-padding pt-28 lg:pt-40">
        <div className="mx-auto max-w-7xl">
          <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8 }} className="mb-16 text-center">
            <p className="mb-4 font-body text-sm tracking-[0.3em] text-primary">WHEN TO FIND US</p>
            <h2 className="font-display text-4xl font-bold md:text-5xl">Two windows, <span className="glow-text">two moods</span></h2>
          </motion.div>
          <div className="mx-auto grid max-w-4xl gap-6 md:grid-cols-2">
            {schedule.map((s, i) => (
              <motion.div key={s.day} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: i * 0.15 }} className="glass-card p-8 text-center">
                ...
                <p className="mt-4 font-body text-sm text-muted-foreground">{s.day.split("—")[1]?.trim()}</p>
                <div className="my-4 flex items-center justify-center gap-2">
                  <Clock className="h-5 w-5 text-primary" />
                  <p className="font-display text-2xl font-bold">{s.time}</p>
                </div>
                <p className="font-body text-sm leading-relaxed text-muted-foreground">{s.vibe}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="section-padding pt-28 lg:pt-40">
        <div className="mx-auto max-w-3xl text-center">
          <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8 }}>
            <p className="mb-4 font-body text-sm tracking-[0.3em] text-primary">THE PHILOSOPHY</p>
            <h2 className="mb-6 font-display text-4xl font-bold md:text-5xl">Herbs you know, <span className="glow-text">results you'll feel</span></h2>
            <p className="font-body text-base leading-relaxed text-muted-foreground">
              Everything at the Bio Bar starts with herbs your grandmother used — lemongrass, ginger, hibiscus, chamomile, turmeric, aloe vera. We mix them with everyday ingredients into drinks that taste amazing. How we blend them, what ratios we use, and the little details that make each one hit differently? You'll have to show up to find out.
            </p>
            <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
              {["Fresh Herbs", "Made On-Site", "Event Exclusive", "Recipes Stay With Us"].map((tag) => (
                <span key={tag} className="rounded-full border border-primary/20 bg-primary/5 px-4 py-2 font-body text-xs tracking-[0.15em] text-primary">{tag}</span>
              ))}
            </div>
          </motion.div>
        </div>
      ...
      <section className="section-padding pt-28 lg:pt-40 relative">
        <div className="absolute left-1/2 top-0 h-px w-3/4 -translate-x-1/2 bg-gradient-to-r from-transparent via-warm-rose/20 to-transparent" />
        <div className="mx-auto max-w-7xl">
          <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8 }} className="mb-16 text-center">
            <div className="mb-4 flex items-center justify-center gap-2">
              <FlaskConical className="h-4 w-4 text-warm-rose" />
              <p className="font-body text-sm tracking-[0.3em] text-warm-rose">COMING SOON</p>
            </div>
            <h2 className="mb-4 font-display text-4xl font-bold md:text-5xl">
              Curated <span className="glow-text">Herbs</span>
            </h2>
            <p className="mx-auto max-w-2xl font-body text-base text-muted-foreground">
              We're building something special — a curated selection of premium herbs and botanicals for your own wellness rituals at home. The same quality. The same intention. Coming to you soon.
            </p>
          </motion.div>

          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 max-w-4xl mx-auto">
            {curatedHerbs.map((herb, i) => (
              <motion.div
                key={herb.name}
                initial={{ opacity: 0, y: 30 }}
                ...
                className="glass-card p-6 text-center transition-all duration-300 hover:border-warm-rose/30"
              >
                <span className="text-2xl mb-3 block">{herb.icon}</span>
                <h3 className="font-display text-lg font-bold mb-1">{herb.name}</h3>
                <p className="font-body text-xs text-muted-foreground">{herb.benefit}</p>
              </motion.div>
            ))}
          </div>

          <div className="mt-12 text-center">
            <span className="inline-block rounded-full border border-warm-rose/30 bg-warm-rose/5 px-6 py-2 font-body text-xs tracking-[0.2em] text-warm-rose">
      ...
      <section className="section-padding pt-28 lg:pt-40 relative">
        ...
        <div className="mx-auto max-w-7xl">
          <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8 }} className="mb-20 text-center">
            <div className="mb-4 flex items-center justify-center gap-2">
              <Droplets className="h-4 w-4 text-primary" />
              <p className="font-body text-sm tracking-[0.3em] text-primary">THE MENU</p>
            </div>
            <h2 className="mb-4 font-display text-4xl font-bold md:text-5xl">What's <span className="glow-text">on tap</span></h2>
            <p className="mx-auto max-w-2xl font-body text-base text-muted-foreground">
              Six herbal blends — each built around real plants you already know. We'll tell you what it does. The recipe? That's ours.
            </p>
          </motion.div>
      ...
      <section className="section-padding pt-20 lg:pt-28">
        <div className="mx-auto max-w-3xl">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className="glass-card border-primary/20 p-8 md:p-12">
            <div className="mb-4 flex items-center gap-3">
              <Leaf className="h-5 w-5 text-primary" />
              <h3 className="font-display text-xl font-bold">You Have to Be There</h3>
            </div>
            <div className="space-y-4 font-body text-sm leading-relaxed text-muted-foreground">
              <p>The Bio Bar is only available during CoppahandGold events. Every drink is prepared fresh on-site. No pre-made mixes, no bottled shortcuts, no recipes shared online.</p>
              <p>All drinks are complimentary for event attendees. The only way to taste them is to be in the room.</p>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="section-padding relative overflow-hidden">
        ...
        <div className="mx-auto max-w-2xl text-center">
          <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
            <h2 className="mb-6 font-display text-4xl font-bold md:text-5xl">Taste it <span className="glow-text">yourself</span></h2>
            <p className="mb-10 font-body text-base leading-relaxed text-muted-foreground">
              The Bio Bar is part of the full CoppahandGold experience. Every sip is included — but you have to be there.
            </p>
            <a href="/experience" className="group relative inline-flex items-center gap-2 overflow-hidden rounded-full bg-primary px-8 py-4 font-body text-sm font-semibold tracking-wide text-primary-foreground transition-all duration-500 hover:shadow-[0_0_40px_hsl(165_80%_45%/0.4)]">
      ...
      <footer className="border-t border-border/30 px-6 py-12 md:px-12">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-6 md:flex-row">
          <p className="font-display text-lg font-bold tracking-wider"><span className="glow-text">COPPAHANDGOLD</span></p>
          <p className="font-body text-xs text-muted-foreground">© 2025 CoppahandGold. Abuja, Nigeria. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
